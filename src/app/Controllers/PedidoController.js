...
import RegistroPedido from "./model/RegistroPedido";

// const {
//   calcularFrete
// } = require("./integracoes/correios");
import PagamentoValidation from "./validacoes/pagamentoValidation";
import EntregaValidation from "./validacoes/entregaValidation";
import QuantidadeValidation from "./validacoes/quantidadeValidation";

import EmailController from "./EmailController";

import CarrinhoValidation from "./validacoes/carrinhoValidation";
...
//   }
  ...
  async store(req, res, next) {
    ...
    try {
      ...
      if (!await CarrinhoValidation(carrinho)) return res.status(422).json({
      ...
      if (!await QuantidadeValidation.validarQuantidadeDisponivel(carrinho)) return res.status(400).send({
        error: "Produtos não tem quantidade disponivel"
      });

      // CHECAR DADOS DE ENTREGA
      if (!await EntregaValidation.checarValorPrazo(cliente.endereco.CEP, carrinho, entrega)) return res.status(422).send({
        error: "Dados de Entrega Inválidos"
      });

      // CHECAR DADOS DO PAGAMENTO
      if (!await PagamentoValidation.checarValorTotal({
          carrinho,
          entrega,
          pagamento
      ...
      if (!PagamentoValidation.checarCartao(pagamento)) return res.status(422).send({
        error: "Dados de Pagamento com Cartao Inválidos"
      });
      ...
      EmailController.enviarNovoPedido({
        pedido,
